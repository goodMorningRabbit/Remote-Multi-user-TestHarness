...
int main()
{
  ...
  std::cout << "\n  address = " << ep.address;std::cout << "\n  port = " << ep.port;
  std::string epStr = ep.toString();std::cout << "\n  " << epStr;

  EndPoint newEp = EndPoint::fromString(epStr);
  std::cout << "\n  " << newEp.toString();Utilities::putline();

  Utilities::title("testing messages");Utilities::putline();

  Utilities::title("creating message from Message::methods");
  Message msg;msg.name("msg#1");msg.to(EndPoint("localhost", 8080));
  msg.from(EndPoint("localhost", 8081));msg.command("doIt");msg.contentLength(42);msg.file("someFile");msg.show();

  Utilities::title("testing Message msg = fromString(msg.toString())");
  Message newMsg = Message::fromString(msg.toString());newMsg.show();
  ...
  Utilities::title("adding custom attribute");newMsg.attribute("customName", "customValue");newMsg.show();

  Utilities::title("testing assignment");
  Message srcMsg;srcMsg.name("srcMsg");srcMsg.attribute("foobar", "feebar");srcMsg.show();
  std::cout << "\n  assigning srcMsg to msg #1";newMsg = srcMsg;newMsg.show();
