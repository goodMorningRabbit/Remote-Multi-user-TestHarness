...
bool SocketConnecter::connect(const std::string& ip, size_t port)
{
  ...
  if (iResult != 0) {
    Show::write("\n  -- getaddrinfo failed with error: " + Conv<int>::toString(iResult));return false;
  }
  ...
  for (ptr = result; ptr != NULL; ptr = ptr->ai_next) {
    ...
    if (ptr->ai_family == AF_INET) { // IPv4
	  ...
	  addr = &(ipv6->sin6_addr);ipver = "IPv6";
    }
    ...
    if (socket_ == INVALID_SOCKET) {
      int error = WSAGetLastError();Show::write("\n\n  -- socket failed with error: " + Conv<int>::toString(error));
      return false;
    }
    iResult = ::connect(socket_, ptr->ai_addr, (int)ptr->ai_addrlen);
    if (iResult == SOCKET_ERROR) {
      socket_ = INVALID_SOCKET;int error = WSAGetLastError();
      Show::write("\n  -- WSAGetLastError returned " + Conv<int>::toString(error));continue;
    }break;
  }
  ...
  if (socket_ == INVALID_SOCKET) {
    ...
    Show::write("\n  -- unable to connect to server, error = " + Conv<int>::toString(error));return false;
  }
  return true;
}
/////////////////////////////////////////////////////////////////////////////
...
void clientTestBufferHandling(Socket& si)
{
  ...
  si.sendString(command);Show::write("\n  client sent : " + command);
  const int BufLen = 20;
  Socket::byte buffer[BufLen];
  for (size_t i = 0; i < 5; ++i){
    std::string text = "Hello World " + std::string("#") + Conv<size_t>::toString(i + 1);
    ...
    buffer[BufLen - 1] = '\0';si.send(BufLen, buffer);
  ...
  bool ok;std::string collector;
  while (true){
    if (si.bytesWaiting() == 0)
      break;
    ok = si.recv(BufLen, buffer);
    if (!ok){
      Show::write("\n  client unable to receive");break;
    }
