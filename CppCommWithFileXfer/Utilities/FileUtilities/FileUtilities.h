...
namespace Utilities
{
  ...
  inline bool openReadFile(const PathStr& fileSpec, std::ifstream& in)
  {
	  in.open(fileSpec);
	  if (!in.good())
	  {
		  std::cout << "\n  can't open file \"" << fileSpec << "\" for reading";
		  return false;
	  }
	  return true;
  }
  ...
  inline bool openWriteFile(const PathStr& fileSpec, std::ofstream& out)
  {
	  out.open(fileSpec);
	  if (!out.good())
	  {
		  std::cout << "\n  can't open file \"" << fileSpec << "\" for writing";
		  return false;
	  }
	  return true;
  }
