...
{
  Title("Testing CodeUtilities");putline();
  title("Testing ProcessCmdLine");
  ProcessCmdLine pcl(argc, argv);pcl.usage();
  ...
  preface("MaxItems: "); pcl.showMaxItems();std::cout << "\n";

  preface("changing path: ");pcl.path("c:\\su");pcl.showPath();
  preface("adding option: ");pcl.option('/q');pcl.showOptions();
  preface("adding pattern: ");pcl.pattern("*.htm");pcl.showPatterns();

  preface("changing maxItems: ");pcl.maxItems(15);
  pcl.showMaxItems();putline();
  preface("new command line: ");pcl.showCmdLine();putline();

  title("Testing Box and PersistFactory");
  Box<double> boxedDouble = 3.1415927;
  std::cout << "\n  boxedDouble = " << boxedDouble;boxedDouble = 0.33333;
  std::cout << "\n  boxedDouble = " << boxedDouble;

  PersistFactory<std::string> name = std::string("Jim");name.append(" Fawcett");
  std::cout << "\n  " << name.toXml("name");

  PersistFactory<Box<double>> value = Box<double>(9.87654);
  std::cout << "\n  " << value.toXml("value");value += 3.14159;
  std::cout << "\n  " << value.toXml("value");std::cout << "\n\n";
  ...
  std::cout << conv1 << ", " << conv2 << ", " << conv3;putline();
  ...
  std::cout << Converter<std::string>::toValue(conv3);putline(2);
  return 0;
}
#endif
