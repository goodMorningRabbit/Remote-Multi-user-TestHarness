...
int main()
{
  ...
  using ILogger = ILogger<1, Lock>;using Logger = Logger<1, Lock>;using LoggerFactory = SingletonLoggerFactory<1, Lock>;
  ...
  std::string head = "\n  Demonstration Log";
  pLogger->writeHead(head);pLogger->write("  Hi from main");pLogger->write("  hi again");pLogger->writeTail();
  outfileStrm.close();
